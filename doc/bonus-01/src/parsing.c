...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/14 00:30:37 by clem              #+#    #+#             */
/*   Updated: 2022/02/21 17:01:02 by cvidon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_set_path(t_data *data)
{
	...
	if (data->env[i - 1] == NULL)
		exit(ft_perr(data, __FILE__, __LINE__, "Error path"));
}
...
void	ft_set_prg(t_data *data)
{
	...
	prg = ft_extract_prg(data->av[2 + data->cmdid]);
...
{
	data->args[data->cmdid] = ft_split(data->av[2 + data->cmdid], ' ');
}
