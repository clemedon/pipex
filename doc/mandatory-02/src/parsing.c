...
void	ft_set_prg(t_data *data)
{
	...
	prg = ft_extract_prg(data->av[2 + data->cmdcount]);
	data->prgs[data->cmdcount] = ft_strdup(prg);
	i = -1;
	while (data->path[++i])
	{
...
{
	data->args[data->cmdcount] = ft_split(data->av[2 + data->cmdcount], ' ');
}
